...
QList<AChartDecorator *> *AChartDecoratorContainer::group(int groupeNum)
{
  if ( (groupeNum >= 0) && (groupeNum < m_groupeDecorators.count()) )
    return m_groupeDecorators.values().at(groupeNum);
  else return NULL;
}
  ...
  while (!groupeIn.atEnd())
    {
      ...
      QString groupeName;
      groupeIn >> groupeName;
      if (!containsGroupe(groupeName)) addGroup(groupeName);
      // создаём декораторы
      QByteArray groupeData;
      groupeIn >> groupeData;
...
//              m_groupeDecorators << decorator;
            ...
            {
              QMessageBox::warning(NULL, tr("Вы не добавили объект с нужным типом в AChartDecoratorContainer?"),
  ...
  for (int numGroupe = 0;numGroupe < m_groupeDecorators.count();numGroupe++)
    {
      ...
      while (i.hasNext())
        {
//          i.next()->deleteLater();
          delete(i.next());//вроде без ошибок, но в общем напрягает
        }
    }
}
