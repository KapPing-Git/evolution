...
//  m_startX = 0;
  ...
  m_anyPointChanged = false;
  m_useOptimisation = true;
}
...
//        }
      addPointToOptimalSeries(QPointF(x,y));
    }
...
{
  if (m_realPolyline->isEmpty()) return;
...
//  qreal screenPerPhysX = (qreal)screenWidth / physicalView.width();
  m_screenPerPhysX = (qreal)screenWidth / physicalView.width();
  m_prevScreenWidth = 0;//(m_realPolyline->at(0).x()) * m_screenPerPhysX;
  m_pointMax = m_realPolyline->at(0);
  m_pointMin = m_realPolyline->at(0);
...
//  float startX = m_realPolyline->at(0).x();
  for(int i=0;i<m_realPolyline->count();i++)
    {
      addPointToOptimalSeries(m_realPolyline->at(i));
...
void AChartSeries::stopTransaction()
{
  ...
  // если любая точка изменена, то надо заново обновить минимумы и максимумы
  if (m_anyPointChanged)
    {
      if (!m_realPolyline->isEmpty())
        {
          m_minX = m_realPolyline->at(0).x();
          m_maxX = m_realPolyline->at(0).x();
          m_minY = m_realPolyline->at(0).y();
          m_maxY = m_realPolyline->at(0).y();
        }
      for (int i = 0;i < m_realPolyline->count();i++)
        {
          qreal x = m_realPolyline->at(i).x();
          qreal y = m_realPolyline->at(i).y();
          if (x < m_minX) m_minX = x;
          if (x > m_maxX) m_maxX = x;
          if (y < m_minY) m_minY = y;
          if (y > m_maxY) m_maxY = y;
...
qreal AChartSeries::height()
{
  return qAbs(maxY() - minY());
}

qreal AChartSeries::width()
{
  return qAbs(maxX() - minX());
}
...
bool AChartSeries::isEmpty()
{
  return m_realPolyline->isEmpty();
}
...
void AChartSeries::addPointToOptimalSeries(QPointF point)
{
  int screenWidth = (point.x() - m_realPolyline->at(0).x()) * m_screenPerPhysX;
  if (screenWidth > m_prevScreenWidth)
    {
      if (m_pointMin.x() > m_pointMax.x())
        {
          m_optimalPolyline->append(m_pointMax);
          m_optimalPolyline->append(m_pointMin);
        }
      else
        {
          m_optimalPolyline->append(m_pointMin);
...
//      qDebug() << "con" << endl;
    ...
    {
      if (point.y() > m_pointMax.y()) m_pointMax = point;
      if (point.y() < m_pointMin.y()) m_pointMin = point;
//      qDebug() << "mid" << endl;
    }
}

bool AChartSeries::getUseOptimisation() const
{
  return m_useOptimisation;
}

void AChartSeries::setUseOptimisation(bool useOptimisation)
{
  m_useOptimisation = useOptimisation;
}
