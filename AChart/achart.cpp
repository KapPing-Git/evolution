...
void AChart::addSeries(AChartSeries *series, QString nameAxis)
{
  ...
  connect(series,SIGNAL(changed()),SLOT(seriesChanged()));
...
int AChart::seriesAxisNum(AChartSeries *series)
{
  //перебираем оси, находим график
  for (int i = 0;i < m_axisContainer->count();i++)
    {
...
//  painter.fillRect(chartRect(),m_chartBackColor);
  painter.fillRect(QRect(0,0,device->width(),device->height()),m_chartBackColor);
  painter.end();
}
...
{
  if (m_axisContainer->isEmpty()) return;
...
{
  if (m_axisContainer->isEmpty()) return QPointF(0,0);
...
{
  if (m_axisContainer->isEmpty()) return QPoint(0,0);
...
//  if (chartRect() == geometry()) //если chartRect() ещё неизвестен определяем его размер
//    drawAxis(device);
      ...
      for (int numAxis = 0;numAxis < m_axisContainer->count();numAxis++)
        {
          AChartAxis *axis = m_axisContainer->axis(numAxis);
          AChartSeriesCollection *seriesCollection = axis->seriesCollection();
          qreal realMinX = seriesCollection->minX();
          qreal realMaxX = seriesCollection->maxX();
          qreal realMinY = seriesCollection->minY();
          qreal realMaxY = seriesCollection->maxY();
          //если ширина оси больше минимально допустимой
          qreal minX,minY,maxX,maxY;
          if (seriesCollection->width() > axis->autoChangeMinimumFhysicalViewSize().width())
            {
              minX = realMinX - screenToPhysicalWidth(m_viewModeMarginsX,axis);
              maxX = realMaxX + screenToPhysicalWidth(m_viewModeMarginsX,axis);
            }
          else
            {
              qreal midX = realMinX + (realMaxX - realMinX) / 2;
              minX = midX - axis->autoChangeMinimumFhysicalViewSize().width() / 2;
              maxX = midX + axis->autoChangeMinimumFhysicalViewSize().width() / 2;
            }
          //если высота оси больше минимально допустимой
          if (seriesCollection->height() > axis->autoChangeMinimumFhysicalViewSize().height())
            {
              minY = realMinY - screenToPhysicalHeight(m_viewModeMarginsY,axis);
              maxY = realMaxY + screenToPhysicalHeight(m_viewModeMarginsY,axis);
            }
          else
            {
              qreal midY = realMinY + (realMaxY - realMinY) / 2;
              minY = midY - axis->autoChangeMinimumFhysicalViewSize().height() / 2;
              maxY = midY + axis->autoChangeMinimumFhysicalViewSize().height() / 2;
            }
           axis->setPhysicalView(minX,minY,maxX,maxY);
        }
...
AChartMousePhysicalCoord *AChart::mousePhisicalCoort()
{
  return &m_mousePhysicalCoord;
}
