...
bool AChartAxisContainer::isEmpty()
{
  return m_axisCollection.isEmpty();
}

void AChartAxisContainer::deleteAxis(QString nameAxis)
{
  AChartAxis *axis = m_axisCollection[nameAxis];
  delete(axis);
  m_axisCollection.remove(nameAxis);
}

void AChartAxisContainer::clear()
{
  QList<AChartAxis *> axises = m_axisCollection.values();
  for (int i = 0;i < axises.count();i++)
    {
      delete (axises[i]);
    }
  m_axisCollection.clear();
  m_activeAxis = NULL;
}
